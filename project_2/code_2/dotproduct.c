 ...
 * (No-overflow version: integer types use native-width accumulators
 *  without widening to long long.)
 *
...
int DotInt(const int *a, const int *b, int n) {
    int sum = 0;
    for (int i = 0; i < n; i++) {
        sum += a[i] * b[i];
    }
...
int DotShort(const short *a, const short *b, int n) {
    int sum = 0;
    for (int i = 0; i < n; i++) {
        sum += a[i] * b[i];
    }
...
int DotChar(const signed char *a, const signed char *b, int n) {
    int sum = 0;
    for (int i = 0; i < n; i++) {
        sum += a[i] * b[i];
    }
