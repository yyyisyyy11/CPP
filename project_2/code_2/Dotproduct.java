 ...
 * (No-overflow version: integer types use int accumulators
 *  without widening to long.)
...
public class Dotproduct {
    ...
    static int dotShort(short[] a, short[] b) {
        int sum = 0;
        for (int i = 0; i < a.length; i++) {
            sum += a[i] * b[i];
        }
    ...
    static int dotByte(byte[] a, byte[] b) {
        int sum = 0;
        for (int i = 0; i < a.length; i++) {
            sum += a[i] * b[i];
        }
